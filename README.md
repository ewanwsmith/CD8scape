...
- `variant_fates.csv`: Per-stage fate of each variant in `variants.csv`, in long format. Columns: `Frame`, `Locus`, `Mutation` (matching `harmonic_mean_best_ranks.csv`), `source` (upstream pipeline node), `target` (downstream node). Each variant produces one row per stage it reaches; aggregating by `(source, target)` and counting rows gives the link values for a flow diagram. Pipeline nodes: `Input → Out of frame | In frame`, `In frame → Synonymous | Stop codon | Non-synonymous`, `Non-synonymous → Non-binding | Passed`. Written at the end of `run` and `run_supertype`.
