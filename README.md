...
- `variant_fates.csv`: Per-variant pipeline fate in wide format. Columns: `Frame`, `Locus`, `Mutation` (matching `harmonic_mean_best_ranks.csv`), `frame_filter` (`In frame` | `Out of frame`), `peptide_filter` (`Non-synonymous` | `Synonymous` | `Stop codon` | missing), `binding_filter` (`Passed` | `Non-binding` | missing). Each adjacent pair of stage columns gives a source→target relationship for a flow diagram. Written at the end of `run` and `run_supertype`.
